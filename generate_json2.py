...
def transform_for_altstore(original_object):
    ...
    for app in filter_unique_values(original_object["apps"], "bundleIdentifier"): # altstore doesn't like duplicated bundle ids
        ...
        app_map[name]["versions"].append(
            {
                "version": version,
                "date": f"{version_date}T00:00:00-00:00",
                "size": size,
                "downloadURL": download_url,
                "localizedDescription": localized_description,
            }
...
if __name__ == "__main__":
    ...
    with open("apps_altstore.json", "w") as file:
        json.dump(transform_for_altstore(data), file, indent=2)
